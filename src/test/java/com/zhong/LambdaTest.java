...
public class LambdaTest {
    ...
    /**
     * @author juzi
     * @date 2023/5/22 下午 6:13
     * @description 从对象list中取出最大值
     */
    @Test
    public void max2() {
        List<EntityTest> list = Lists.newArrayList(
                EntityTest.builder().id(1L).build(),
                EntityTest.builder().id(4L).build(),
                EntityTest.builder().id(6L).build(),
                EntityTest.builder().id(9L).build(),
                EntityTest.builder().id(9L).build()
        );

        Long max = list.stream().map(EntityTest::getId).reduce(Long::max).orElse(0L);
